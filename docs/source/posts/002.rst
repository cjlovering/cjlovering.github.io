...
Beam search is a method for decoding a sequence given an auto-regressive function that outputs a probability distribution over the next possible symbols. Ideally, a search algorithm would traverse the different possible paths and select the most probable sequence. However, this would be prohibatively expensive. 
...
Beam search works iteratively. Naturally, the details depend on the decoding function: a hidden markov model with a memory of 1 consumes the previous symbol :cite:`markov`; recurrent neural networks are stateful :cite:`cho2014learning`; and transformer networks consume the entire prefix :cite:`vaswani2017attention`.
