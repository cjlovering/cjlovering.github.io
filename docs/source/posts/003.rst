...
First, an input embedding for each word is retrieved. TN uses Byte-Encoding Representation with a shared embedding matrix :cite:`sennrich2015neural` -- this itself improves performance. Next a positional encoding is added pointwise to each dimension of the input vector. The identical encoder modules will operate on this representation.
