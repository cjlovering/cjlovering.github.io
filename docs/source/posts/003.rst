...
For future work, it would be interesting to see adaptive computation employed here. Would the additional complexity be of any use or save any computation? It seems like some sentences might require less "decoding".

After decoding the encoded inputs, a linear map is applied to the vectors and a softmax generates an output probability distribution.
