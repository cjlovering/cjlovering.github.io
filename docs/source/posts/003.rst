...
The transformer network uses a stateless auto-regressive strategy which will decode the encoded (but not summarized) source words and the current output words. This allows the model to be parallelized. The authors used scaled dot product attention
