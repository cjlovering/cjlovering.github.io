...
After reviewing the modules in turn, the structure as a whole is transparent. An input embedding for each word is retrieved. TN uses Byte-Encoding Representation with a shared embedding matrix :cite:`sennrich2015neural`.
